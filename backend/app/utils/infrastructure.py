...
class InfrastructureManager:
    ...
    INFRASTRUCTURES = {
        ...
        "delta": {"name": "Delta", "hostname_patterns": ["delta"], "default": False},
        "delta-ai-ncsa": {
            "name": "Delta AI NCSA",
        ...
        "magic-castle-radiant": {
            ...
            "hostname_patterns": ["magic-castle-radiant", "magic-castle", "llmhub"],
            "default": False,
        },
    }
    ...
    def get_infrastructure_from_env(self) -> Optional[str]:
        ...
        for infra_id in self.INFRASTRUCTURES:
            if infra_id.lower() == value_lower:
                logger.info(
                    f"Infrastructure from env {INFRASTRUCTURE_ENV_VAR}={value!r} -> {infra_id}"
                )
                return infra_id
        logger.warning(
            f"Unknown value for {INFRASTRUCTURE_ENV_VAR}={value!r}; known: {list(self.INFRASTRUCTURES.keys())}"
        )
        return None
    ...
    def detect_infrastructure(self) -> Optional[str]:
        ...
        for infra_id, infra_info in self.INFRASTRUCTURES.items():
            for pattern in infra_info.get("hostname_patterns", []):
                if pattern.lower() in hostname_lower:
                    logger.info(
                        f"Detected infrastructure '{infra_id}' from hostname pattern '{pattern}'"
                    )
                    return infra_id
        ...
        logger.warning(
            "Could not detect infrastructure, using 'campus-cluster' as fallback"
        )
    ...
    def get_infrastructure(self, infrastructure: Optional[str] = None) -> str:
        ...
        if infrastructure:
            if infrastructure not in self.INFRASTRUCTURES:
                logger.warning(
                    f"Unknown infrastructure '{infrastructure}', using detected infrastructure"
                )
    ...
    def setup_infrastructure(self, infrastructure: Optional[str] = None) -> Path:
        ...
        if not models_file.exists():
            ...
            if base_models.exists():
                ...
                logger.warning("models.yaml not found in infrastructure or base config")
