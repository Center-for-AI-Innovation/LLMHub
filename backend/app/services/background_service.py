...
class BackgroundService:
    ...
    def __init__(self):
        ...
        self.model_sync_interval = (
            settings.MODEL_SYNC_INTERVAL or 3600
        )  # Default to 1 hour
        self.last_sync = datetime.utcnow()
        self.last_expiry_check = datetime.utcnow()
        self.last_model_sync = datetime.utcnow() - timedelta(
            hours=1
        ...
        logger.info(
            f"Background service initialized with sync_interval={self.sync_interval}s, expiry_check_interval={self.expiry_check_interval}s, model_sync_interval={self.model_sync_interval}s"
        )
        ...
        try:
            with SessionLocal() as db:
                ...
                if result.get("success"):
                    logger.info(
                        f"Successfully synced {result.get('count', 0)} models on startup"
                    )
                else:
                    logger.error(
                        f"Failed to sync models on startup: {result.get('error', 'Unknown error')}"
                    )
    ...
    async def _run_periodic_tasks(self):
        ...
        while self.running:
            ...
            try:
                ...
                # Run expiry check
                if (
                    now - self.last_expiry_check
                ).total_seconds() >= self.expiry_check_interval:
                    await self._check_expired_deployments()
                    self.last_expiry_check = now

                # Run model sync
                if (
                    now - self.last_model_sync
                ).total_seconds() >= self.model_sync_interval:
                ...
                logger.error(
                    f"Error in background task: {e}. Backing off for {backoff_time:.2f}s"
                )
        ...
        try:
            with SessionLocal() as db:
                ...
                if result.get("success"):
                    ...
                    logger.error(
                        f"Failed to sync models: {result.get('error', 'Unknown error')}"
                    )
        except Exception as e:
            ...
            with SessionLocal() as db:
                ...
                if len(active_deployments) > self.max_deployments_per_cycle:
                    ...
                    active_deployments = active_deployments[
                        : self.max_deployments_per_cycle
                    ]

                logger.info(
                    f"Syncing status for {len(active_deployments)} active deployments"
                )

                for deployment in active_deployments:
                    try:
                        updated = self.model_service.update_deployment_status(
                            db, deployment.id
                        )
                        if not updated:
                        ...
                        if updated.status == "ready":
                            ...
                            notification_type = (
                                "completed"
                                if updated.errorMessage == "Slurm job timeout"
                                else "failed"
                            )
                        else:
                        ...
                        logger.error(
                            f"Error updating status for deployment {deployment.id}: {e}"
                        )
        except Exception as e:
    ...
    async def _check_expired_deployments(self):
        ...
        try:
            with SessionLocal() as db:
                ...
                logger.info(
                    f"Found {len(expired_deployments)} expired deployments to shut down"
                )
                ...
                for deployment in expired_deployments:
                    try:
                        ...
                        updated = self.model_service.shutdown_deployment(
                            db, deployment.id
                        )
                        ...
                        logger.error(
                            f"Error shutting down expired deployment {deployment.id}: {e}"
                        )
        except Exception as e:
