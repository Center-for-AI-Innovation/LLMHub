...
def test_verify_access_gated_no_token_mocked():
    with patch("app.utils.hf_auth.HfApi") as MockApi:
        MockApi.return_value.auth_check.side_effect = GatedRepoError(
            "gated", response=_mock_response(403)
        )
...
def test_verify_access_repo_not_found_mocked():
    with patch("app.utils.hf_auth.HfApi") as MockApi:
        MockApi.return_value.auth_check.side_effect = RepositoryNotFoundError(
...
def test_check_access_not_gated():
    ok, err = check_model_hf_access(
        gated=None, huggingface_id="org/model", user_token=None
    )
    assert ok is True
    assert err is None
...
def test_check_access_gated_no_token():
    ok, err = check_model_hf_access(
        gated="manual", huggingface_id="org/model", user_token=None
    )
    assert ok is False
...
def test_check_access_gated_valid_token():
    with patch(
        "app.utils.hf_auth.verify_hf_model_repo_access", return_value=(True, None)
    ):
        ok, err = check_model_hf_access(
            gated="auto", huggingface_id="org/model", user_token="hf_abc"
        )
    assert ok is True
    assert err is None
...
def test_check_access_gated_invalid_token():
    with patch(
        "app.utils.hf_auth.verify_hf_model_repo_access",
        return_value=(False, "access denied"),
    ):
        ok, err = check_model_hf_access(
            gated="auto", huggingface_id="org/model", user_token="bad-token"
        )
    assert ok is False
...
def test_verify_access_gated_model_with_real_token():
    ...
    ok, err = verify_hf_model_repo_access(
        "meta-llama/Meta-Llama-3.1-8B-Instruct", HF_TOKEN
    )
