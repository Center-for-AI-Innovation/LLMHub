...
from app.schemas.resource_allocation import ResourceAllocationCreate, ResourceAllocationUpdate
...
class ResourceService:
    ...
    def create_resource(self, db: Session, resource: ResourceAllocationCreate) -> ResourceAllocation:
    ...
    def get_resource(self, db: Session, resource_id: UUID) -> Optional[ResourceAllocation]:
        """Get a resource allocation by ID."""
        return db.query(ResourceAllocation).filter(ResourceAllocation.id == resource_id).first()
    ...
    def get_resources(
        ...
        return query.order_by(ResourceAllocation.createdAt.desc()).offset(skip).limit(limit).all()
    ...
    def get_resource_summary(self, db: Session) -> Dict[str, Any]:
        ...
        for resource in resources:
            ...
            resource_key = f"{resource.resourceType.lower()}_{resource.resourceName.lower()}"
