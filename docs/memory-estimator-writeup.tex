...
bound over eight historical vLLM~0.11.0 startup probes on Delta (a ninth,
captured live from the production launch path, validates it --- \S\ref{sec:live}) (Qwen2.5-0.5B/7B;
...
\item \textbf{Calibration breadth:} nine probes, two models ($\le$7B), two
...
\item \textbf{One real defect found and fixed}: the job booted with
  ...
  probe so regression tests pin it. (The frontend's copy of the constant is
  updated in the same PR.)
\end{itemize}

\section{Portability to other Slurm clusters}

\code{python -m app.services.fit\_estimator.discovery --output hardware.yaml}
reads partitions, GPU GRES names, and walltimes from \code{sinfo} and emits a
hardware table in the estimator's schema; \code{FIT\_ESTIMATOR\_HARDWARE\_YAML}
activates it. VRAM resolves by (1) \code{--probe} (a tiny \code{srun
nvidia-smi} per GPU type --- measured truth) or (2) a bundled table that
deliberately assumes the \emph{smallest} memory variant for ambiguous GRES
names, so an unprobed cluster can only produce false rejects. Unknown GPUs are
excluded, never guessed. SU rates are site policy and must be hand-edited; the
overhead calibration transfers as-is (it is a property of the vLLM version, not
the GPU). Validated by regenerating Delta's own table from live \code{sinfo}
and reproducing identical verdicts.

\section{Verification status}

158 backend tests pass (including the live production-container probe and the Slurm hardware-discovery suite) (fully offline; real HF configs and the eight probe
